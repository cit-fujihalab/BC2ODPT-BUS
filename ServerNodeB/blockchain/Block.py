...
class Block:
	...
	def to_dict(self,include_nonce= True):
		d = {
			...
			'transactions' : [json.dumps(self.transactions[i], sort_keys=True, ensure_ascii=False) for i in range(len(self.transactions))],#list(map(json.dumps, self.transactions)),
	...
	def _compute_nonce_for_pow(self, message, difficulty=4):
...
#		super().__init__(transactions={"message":"this_is_simple_bitcoin_genesis_block"}, previous_block_hash=None)
#
#	def to_dict(self, include_nonce=True):
#		d = {
#			'timestamp' :self.timestamp,
#			'transactions': json.dumps(self.transactions, sort_keys=True, ensure_ascii=False),
#			'genesis_block': True,
#		}
#		if include_nonce:
#			d['nonce'] = self.nonce
